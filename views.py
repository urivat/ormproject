...
def problem_one(request):
    ...
    student_gpa = Student.objects.filter(gpa__gt=3).order_by('-gpa', 'first_name')
...
def problem_two(request):
    instructors = Instructor.objects.filter(hire_date__year__lt= 2010).order_by('hire_date', 'first_name')
...
def problem_five(request):
    new_student = Student.objects.create(first_name = 'Kyle', last_name = 'Harwood', year = 2022, gpa = 3.0, id = 11)
...
def problem_six(request):
    ...
    student_gpa = Student.objects.filter(pk = 11).update(gpa=3.5)
